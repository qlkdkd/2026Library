...
class DataSearcher:
    def __init__(self):
        ...
        # MySQL 연결 설정 (본인의 DB 정보 입력)
        self.DB_USER = "root"
        self.DB_PASSWORD = "sdh!1025"
        self.DB_HOST = "localhost"
        self.DB_PORT = "3306"
        self.DB_NAME = "library_db"

        self.connection_string = f"mysql+pymysql://{self.DB_USER}:{self.DB_PASSWORD}@{self.DB_HOST}:{self.DB_PORT}/{self.DB_NAME}?charset=utf8mb4"
        self.engine = create_engine(self.connection_string)

        # 코드 정보 데이터
        self.region = pd.read_sql_table('region', con=self.engine)
        self.dtl_region = pd.read_sql_table('detail_region', con=self.engine)
        self.dtl_kdc = pd.read_sql_table('detail_subject', con=self.engine)
        self.kdc = pd.read_sql_table('genre', con=self.engine)
        self.addCode = pd.read_sql_table('isbn_code', con=self.engine)
        self.age = pd.read_sql_table('age_group', con=self.engine)
        self.gender = pd.read_sql_table('gender', con=self.engine)
        ...
        # 도서관 정보
        self.libDf = pd.read_sql_table('library', con=self.engine)
        ...
        self.tag_mapping = {
            ...
            '도서 상세 조회': {'parent': 'detail', 'target': 'book', 'paging': False},
...
if __name__ == '__main__':
    ...
    test = ds.findLibrary('의정부미술도서관')
    print('도서관 코드 찾기 결과:', test)
    print(type(test))
    ...
    test1 = ds.read_xml_by_key('도서관별 장서/대출 데이터 조회', params)
    print('xml 데이터프레임 변환 테스트:')
    print(test1.head())
    test1.to_csv(f'./extracted_data/SciArtLib.csv', index=False)
